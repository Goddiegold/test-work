...
const ProjectsView = ({ viewProgress }) => {
    ...
    const Arr = breakDown();
    const [currentData, setCurrentData] = useState(Arr);
    ...
    const projectsdropdowns = ["All", "Ongoing", "Completed"];
    const handleFilter = (value) => {
        if(value == "All") {
            setCurrentData(Arr);
        }
        else {
            const update = projects.filter(val => val.type == value);
            let updateData = [];
            for(var i=0;i<update.length;i+=6) updateData.push(update.slice(i, i+6));
    ...
    return (
        <div className={viewProgress ? "projects_view" : "projects_view_full"}>
            <div className="projects_view_wrapper">
                <div className="projects_view_content">
                    ...
                    <div className="pagination">
                        <CustomPagination len={currentData.length} handleScroll={handleScroll} />
                    </div>
                </div>
            </div>
        </div>
